package com.cc.personal.llmapidemo.intent;
 ...
 *  不确定的则需要引导澄清
 *
 * 意图识别的实现通常是混合方案：规则层判断闲聊，LLM判断其他意图
 * ：判断意图----规则层命中为闲聊---则走闲聊逻辑（将用户query直接发送给LLM 不带记忆等 减少tokens
 *                          规则层未命中走LLM 判断为知识检索---走向量库检索等---
 *                                          判断为tool工具调用---调用工具 ---
 *
 *
 *
 *  意图分类的准确率监控: 分类准确率；各意图的占比例；误分类的典型案例；规则层命中率
 *  意图边界的动态调整： 后续根据业务调整。 比如都是调用工具，到底是调什么方面的工具   都是知识检索 检索的是哪一领域的知识
 *  意图分类失败兜底：解析失败或者意图不合法则走知识检索
 *
 */
